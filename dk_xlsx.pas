...
uses
  ...
  DK_Vector, DK_Matrix, DK_StrUtils, rxdbgrid;
...
type
  ...
  public
    constructor Create(const AWorksheet: TsWorksheet; const AGrid: TsWorksheetGrid; const AColWidths: TIntVector);
  ...
  //включать в Form.OnShow для предотвращения скачков высот строк
  procedure PrepareGridRowHeights(const AGrid: TsWorksheetGrid;
                                  const AHeight: Integer = ROW_HEIGHT_DEFAULT);
...
type
  ...
  public
    constructor Create(const AWorksheet: TsWorksheet; const AGrid: TsWorksheetGrid;
      const AColWidths: TIntVector; const AFontName: String; const AFontSize: Single;
      const AFontStyle: TFontStyles = []);
    destructor  Destroy; override;
    procedure UpdateColors(const AColorVector: TColorVector);
    procedure ClearColors;
    procedure UpdateRowHeights;
    property XLSX: TXLSX read FX;
  end;
...
var
  i: Integer;
begin
  AGrid.RowHeights[0]:= 1;
  for i:= 1 to AGrid.RowCount-1 do
    AGrid.RowHeights[i]:= AHeight;
end;
...
constructor TXLSXDraw.Create(const AWorksheet: TsWorksheet;
  const AGrid: TsWorksheetGrid; const AColWidths: TIntVector;
  const AFontName: String; const AFontSize: Single;
  const AFontStyle: TFontStyles = []);
var
  XGrid: TsWorksheetGrid;
begin
  inherited Create;
  XGrid:= nil;
  if Assigned(AGrid) then
    if AWorksheet=AGrid.Worksheet then
      XGrid:= AGrid;
  FX:= TXLSX.Create(AWorksheet, XGrid, AColWidths);
  FFontName:= AFontName;
  FFontSize:= AFontSize;
  FFontStyle:= AFontStyle;
end;

destructor TXLSXDraw.Destroy;
begin
  FreeAndNil(FX);
  inherited Destroy;
end;

procedure TXLSXDraw.UpdateColors(const AColorVector: TColorVector);
begin
  FX.ApplyBGColors(AColorVector);
end;

procedure TXLSXDraw.ClearColors;
begin
  FX.ClearBGColors;
end;

procedure TXLSXDraw.UpdateRowHeights;
begin
  FX.SetRows;
end;
...
procedure TXLSX.SetDefaultCellSettings;
begin
  SetFontDefault;
  SetAlignmentDefault;
...
procedure TXLSX.DelCellBGColorIndex(const ARow, ACol: Integer);
var
  i: Integer;
begin
...
procedure TXLSX.AddCellBGColorIndex(const ARow, ACol, AColorIndex: Integer);
var
  V: TIntVector;
begin
  V:= nil;
...
procedure TXLSX.AddCellBGColorIndex(const ARow1, ACol1, ARow2, ACol2, AColorIndex: Integer);
var
  i,j: Integer;
begin
...
procedure TXLSX.ApplyBGColors(const ABGColors: TColorVector);
var
  i, R, C: Integer;
  Cl: TsColor;
begin
...
constructor TXLSX.Create(const AWorksheet: TsWorksheet; const AGrid: TsWorksheetGrid;
...
destructor TXLSX.Destroy;
begin
  inherited Destroy;
end;

procedure TXLSX.Clear;
begin
  FWorksheet.Clear;
  FRowHeights:= nil;
  FBGColorMatrix:= nil;
...
function TXLSX.GetHasGrid: Boolean;
begin
  Result:= (FGrid<>nil);
end;

function TXLSX.GetColCount: Integer;
begin
  Result:= Length(FColWidths) - 2*Ord(HasGrid);
end;

function TXLSX.GetRowCount: Integer;
begin
  Result:= Length(FRowHeights) - 2*Ord(HasGrid);
end;

procedure TXLSX.CellIndex(var ARow, ACol: Integer);
begin
  ARow:= RowIndex(ARow);
  ACol:= ColIndex(ACol);
end;

procedure TXLSX.CellIndex(var ARow1, ACol1, ARow2, ACol2: Integer);
begin
  CellIndex(ARow1, ACol1);
  CellIndex(ARow2, ACol2);
end;

function TXLSX.RowIndex(const ARow: Integer): Integer;
begin
  Result:= ARow+FFirstRow-1;
end;

function TXLSX.ColIndex(const ACol: Integer): Integer;
begin
  Result:= ACol+FFirstCol-1;
end;

procedure TXLSX.SetRowHeight(ARow, AValue: Integer);
begin
  ARow:= RowIndex(ARow);
  SetHeight(ARow, AValue);
end;

procedure TXLSX.SetColWidth(ACol, AValue: Integer);
begin
  ACol:= ColIndex(ACol);
  SetWidth(ACol, AValue)
end;

procedure TXLSX.SetWidth(const ACol, AValue: Integer);
begin
...
procedure TXLSX.SetHeight(const ARow, AValue: Integer);
begin
...
procedure TXLSX.SetLineHeight(const ARow, AHeight: Integer; const AMinValue: Integer = ROW_HEIGHT_DEFAULT);
var
...
procedure TXLSX.SetNewRowHeight(const ARow, AHeight: Integer; const AMinValue: Integer = ROW_HEIGHT_DEFAULT);
var
...
procedure TXLSX.SetFrozenRows(AFixRowCount: Integer);
begin
  if HasGrid then
  begin
...
procedure TXLSX.SetRepeatedRows(ABeginRow, AEndRow: Integer);
begin
...
procedure TXLSX.SetRepeatedCols(ABeginCol, AEndCol: Integer);
begin
...
procedure TXLSX.SetFrozenCols(AFixColCount: Integer);
begin
  if HasGrid then
  begin
...
procedure TXLSX.WriteText(ARow1, ACol1, ARow2, ACol2: Integer; AValue: String;
                        const ABordersType: TCellBorderType = cbtNone;
                        const AWordWrap: Boolean = True;
                        const AAutoHeight: Boolean = False;
                        const AWrapToWordParts: Boolean = False;
...
procedure TXLSX.WriteText(const ARow, ACol: Integer; const AValue: String;
                        const ABordersType: TCellBorderType = cbtNone;
                        const AWordWrap: Boolean = True;
                        const AAutoHeight: Boolean = False;
                        const AWrapToWordParts: Boolean = False;
...
procedure TXLSX.WriteNumber(const ARow, ACol: Integer; const AValue: Double;
                            const ABordersType: TCellBorderType = cbtNone;
                            const AFormatString: String = '');
begin
...
procedure TXLSX.WriteNumber(ARow1, ACol1, ARow2, ACol2: Integer; const AValue: Double;
                          const ABordersType: TCellBorderType = cbtNone;
                          const AFormatString: String = '');
begin
  CellIndex(ARow1, ACol1, ARow2, ACol2);
...
procedure TXLSX.WriteCurrency(const ARow, ACol: Integer; const AValue: Double;
                          const ABordersType: TCellBorderType = cbtNone;
                          const AFormatString: String = '');
begin
...
procedure TXLSX.WriteCurrency(ARow1, ACol1, ARow2, ACol2: Integer; const AValue: Double;
                          const ABordersType: TCellBorderType = cbtNone;
                          const AFormatString: String = '');
begin
  CellIndex(ARow1, ACol1, ARow2, ACol2);
...
procedure TXLSX.WriteDate(const ARow, ACol: Integer; const AValue: TDate;
                        const ABordersType: TCellBorderType = cbtNone;
                        const AFormatString: String = '');
begin
...
procedure TXLSX.WriteDate(ARow1, ACol1, ARow2, ACol2: Integer; const AValue: TDate;
                        const ABordersType: TCellBorderType = cbtNone;
                        AFormatString: String = '');
begin
  CellIndex(ARow1, ACol1, ARow2, ACol2);
...
procedure TXLSX.WriteTime(const ARow, ACol: Integer; const AValue: TDateTime;
...
procedure TXLSX.WriteTime(ARow1, ACol1, ARow2, ACol2: Integer; const AValue: TDateTime;
                        const ABordersType: TCellBorderType = cbtNone;
                        AFormatString: String = '');
begin
  CellIndex(ARow1, ACol1, ARow2, ACol2);
...
procedure TXLSX.WriteDateTime(const ARow, ACol: Integer; const AValue: TDateTime;
  const ABordersType: TCellBorderType; const AFormatString: String);
begin
  WriteDateTime(ARow, ACol, ARow, ACol, AValue, ABordersType, AFormatString);
end;

procedure TXLSX.WriteDateTime(ARow1, ACol1, ARow2, ACol2: Integer;
...
function TXLSX.CalcCellHeight(var AText: String; const ACol1, ACol2: Integer;
                             const AWrapToWordParts: Boolean = False;
                             const ARedStrWidth: Integer = 0): Integer;
var
  CellWidth: Integer;
...
begin
  ...
  Result:= STextToCell(AText, Font, CellWidth, ARedStrWidth, AWrapToWordParts, HyphSymbol);
  FreeAndNil(Font);
end;

procedure TXLSX.SetGridRowCount(const ACount: Integer);
begin
...
procedure TXLSX.SetGridColCount(const ACount: Integer);
begin
...
procedure TXLSX.SetCellMainSettings(const ARow, ACol: Integer; const AWordWrap: Boolean);
begin
...
procedure TXLSX.GetBordersNeed(const ABordersType: TCellBorderType;
                           out ALeft, ARight, ATop, ABottom, AInner: Boolean);
begin
  ALeft:= False;
  ARight:= False;
...
procedure TXLSX.SetCellSettings(const ARow1, ACol1, ARow2, ACol2, ARowHeight: Integer;
  const AWordWrap: Boolean; const ABordersType: TCellBorderType);
var
...
procedure TXLSX.DrawCellBorders(const ARow, ACol: Integer;
               const ALeftNeed: Boolean; const ALeftStyle: TsLineStyle; const ALeftColor: TsColor;
               const ARightNeed: Boolean; const ARightStyle: TsLineStyle; const ARightColor: TsColor;
               const ATopNeed: Boolean; const ATopStyle: TsLineStyle; const ATopColor: TsColor;
               const ABottomNeed: Boolean; const ABottomStyle: TsLineStyle; const ABottomColor: TsColor);
...
procedure TXLSX.DrawBorders(ARow, ACol: Integer; const ABordersType: TCellBorderType);
var
  ALeft, ARight, ATop, ABottom, AInner: Boolean;
begin
  CellIndex(ARow, ACol);
...
procedure TXLSX.DrawBorders(const ARow, ACol: Integer;
                          const ALeftNeed: Boolean = True; const ARightNeed: Boolean = True;
                          const ATopNeed: Boolean = True; const ABottomNeed: Boolean = True);
begin
...
procedure TXLSX.DrawBorders(ARow1, ACol1, ARow2, ACol2: Integer;
...
procedure TXLSX.DrawBorders(const ARow1, ACol1, ARow2, ACol2: Integer;
                          const ALeftNeed: Boolean = True; const ARightNeed: Boolean = True;
                          const ATopNeed: Boolean = True; const ABottomNeed: Boolean = True;
                          const AInnerNeed: Boolean = False);
var
...
procedure TXLSX.SetBordersStyle(const ALeftStyle,ARightStyle,ATopStyle,ABottomStyle,AInnerStyle: TsLineStyle);
begin
...
procedure TXLSX.SetBordersColor(const ALeftColor,ARightColor,ATopColor,ABottomColor,AInnerColor: TColor);
begin
...
procedure TXLSX.SetBordersColor(const AAllColor: TColor);
begin
  SetBordersColor(AAllColor, AAllColor, AAllColor, AAllColor, AAllColor);
end;

procedure TXLSX.SetBordersColor(const AAllColor: TsColor);
begin
  SetBordersColor(AAllColor, AAllColor, AAllColor, AAllColor, AAllColor);
end;

procedure TXLSX.SetBordersColor(const ALeftColor,ARightColor,ATopColor,ABottomColor,AInnerColor: TsColor);
begin
...
procedure TXLSX.SetBorders(const AAllStyle: TsLineStyle; const AAllColor: TsColor);
begin
  SetBordersStyle(AAllStyle, AAllStyle, AAllStyle, AAllStyle, AAllStyle);
  SetBordersColor(AAllColor, AAllColor, AAllColor, AAllColor, AAllColor);
end;

procedure TXLSX.SetBorders(const AOuterStyle: TsLineStyle; const AOuterColor: TsColor;
...
procedure TXLSX.SetBorders(const AAllStyle: TsLineStyle; const AAllColor: TColor);
begin
  SetBorders(AAllStyle, ColorGraphicsToFPS(AAllColor));
end;

procedure TXLSX.SetBorders(const AOuterStyle: TsLineStyle; const AOuterColor: TColor;
...
procedure TXLSX.SetBordersDefault;
begin
  SetBorders(BORDER_STYLE_DEFAULT, BORDER_COLOR_DEFAULT);
end;

procedure TXLSX.SetAlignmentDefault;
begin
  SetAlignment(ALIGN_HOR_DEFAULT, ALIGN_VERT_DEFAULT);
end;

procedure TXLSX.SetAlignment(const AHorAlignment: TsHorAlignment;
...
procedure TXLSX.SetBackgroundClear;
begin
   SetBackground(fsNoFill, scTransparent, scTransparent);
end;

procedure TXLSX.SetBackgroundDefault;
begin
  SetBackground(BG_STYLE_DEFAULT, BG_COLOR_DEFAULT, PATTERN_COLOR_DEFAULT);
end;

procedure TXLSX.SetBackground(const ABGStyle: TsFillStyle; const ABGColor: TsColor;
...
procedure TXLSX.SetBackground(const ABGStyle: TsFillStyle; const ABGColor: TColor;
...
procedure TXLSX.SetBackground(const ABGColor: TsColor);
begin
  SetBackground(fsSolidFill, ABGColor, ABGColor);
end;

procedure TXLSX.SetBackground(const ABGColor: TColor);
begin
  SetBackground(fsSolidFill, ABGColor, ABGColor);
end;

procedure TXLSX.SetFontDefault;
begin
  SetFont(FONT_NAME_DEFAULT, FONT_SIZE_DEFAULT,
          FONT_STYLE_DEFAULT, FONT_COLOR_DEFAULT);
end;

procedure TXLSX.SetFontName(const AName: String);
begin
...
procedure TXLSX.SetFont(const AName: String; const ASize: Single;
...
procedure TXLSX.SetFont(const AName: String; const ASize: Single; const AStyle: TFontStyles;
  const AColor: TColor);
begin
  SetFontName(AName);
  FFontSize:= ASize;
...
procedure TXLSX.SetFont(const AFont: TFont);
begin
